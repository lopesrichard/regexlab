...
function setResult() {
  if (from.value.length <= 0 || to.value.length <= 0) return false;

  if (parseInt(from.value) > parseInt(to.value)) return false;

  const ajax = new Ajax(`api/v1.php/${from.value}/${to.value}`);
  ajax
